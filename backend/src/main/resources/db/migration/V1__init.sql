-- ============================================================
-- Flyway V1 — 工单审批流转系统初始 schema
-- 基于 backend/src/main/resources/sql/schema-init.sql
-- 修复点：补全 sys_password_history 表（C-08，原 schema 缺失）
-- ============================================================
...
-- ============================================================
-- 密码历史表（C-08 修复：原 schema 缺失，UserMapper.xml 引用但表不存在）
...
-- 管理层角色（董事长/总经理/副总/总监级）：查看+审批权限
INSERT INTO sys_role_permission (role_id, permission_id, create_time)
SELECT r.id, p.id, NOW()
FROM (SELECT 10 AS id UNION SELECT 11 UNION SELECT 12 UNION SELECT 20 UNION SELECT 21
      UNION SELECT 22 UNION SELECT 23 UNION SELECT 24 UNION SELECT 25) r
